...
var CodexReasoningAdapter = class extends LlmAdapter {
	...
	ultra;
	ultraRoutes;
	constructor(standard, ultra, ultraRoutes) {
		super();
		this.standard = standard;
		this.ultra = ultra;
		this.ultraRoutes = ultraRoutes;
	}
	...
	async resolveModel(provider, model, signal) {
		...
		if (!this.ultraRoutes.has(provider) || reasoning === void 0) return resolved;
	...
	stream(options) {
		...
		if (!this.ultraRoutes.has(options.provider)) throw new LlmError(`provider "${options.provider}" model "${options.model}" does not support reasoning effort "ultra"`, "UNSUPPORTED_REASONING_EFFORT");
...
var DshAuthEveryingSession = class {
	...
	async persist(item) {
		...
		if (useOfficial && official.liveModelsUrl !== void 0) {
			...
			if (token !== void 0) try {
				const { fetchLiveModelIds } = await import("./live-models-0cG49wiE.js");
		...
		if (item.origin === "CC Switch" && item.platform === "codex" && item.baseURL !== void 0) {
			const token = item.credential.type === "oauth" ? item.credential.access : item.credential.key;
			if (token !== void 0) try {
				const { fetchLiveModelIds, modelListingUrls } = await import("./live-models-0cG49wiE.js");
				for (const url of modelListingUrls(item.baseURL)) try {
					const live = await fetchLiveModelIds(url, token);
					available = [.../* @__PURE__ */ new Set([...available, ...live])];
					break;
				} catch {}
			} catch {}
		}
