...
import { LlmAdapter, LlmError, ReasoningEffortId, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
function isCcSwitchCodex(route) {
...
function supportsUltra(route, model) {
	if (!isCcSwitchCodex(route)) return false;
	if (model === void 0) return route.enabled.some((id) => supportsUltra(route, id));
	const declared = route.modelReasoningEfforts?.[model];
	if (declared !== void 0) return declared.some((effort) => effort.trim().toLowerCase() === "ultra");
...
function configuredReasoning(route, id, codexReasoningWire) {
	if (isCcSwitchCodex(route)) return {
		reasoning: true,
		thinkingLevelMap: {
			off: null,
			minimal: null,
	...
	if (route.configuredModel !== id || route.modelReasoningEffort === void 0) return { reasoning: true };
	const effort = route.modelReasoningEffort.trim().toLowerCase();
	if (effort === "off" || effort === "none" || effort === "disabled") return { reasoning: false };
	const level = effort === "ultra" ? "xhigh" : THINKING_LEVELS.includes(effort) ? effort : void 0;
	if (level === void 0) return { reasoning: true };
	const thinkingLevelMap = { off: null };
...
function materializeModel(route, id, codexReasoningWire) {
	return {
		id,
		name: id,
		api: route.api,
...
function customProvider(route, codexReasoningWire = "standard") {
...
async function assertOwnerOnly(filename) {
	...
	if ((mode & 63) !== 0) throw new Error(`dsh-auth-everying: ${filename} is readable beyond its owner (mode ${(mode & 511).toString(8)}); run chmod 600`);
}
...
function parseDocument(text, filename) {
	...
	for (const [key, route] of Object.entries(routes)) {
		...
		parsedRoutes[key] = {
			...
			...typeof route["configuredModel"] === "string" && route["configuredModel"].length > 0 ? { configuredModel: route["configuredModel"] } : {},
			...typeof route["modelReasoningEffort"] === "string" && route["modelReasoningEffort"].length > 0 ? { modelReasoningEffort: route["modelReasoningEffort"] } : {},
			...isRecord(route["modelReasoningEfforts"]) ? { modelReasoningEfforts: Object.fromEntries(Object.entries(route["modelReasoningEfforts"]).flatMap(([id, efforts]) => {
				if (!Array.isArray(efforts)) return [];
				const values = efforts.filter((effort) => typeof effort === "string" && effort.length > 0);
...
function authEveryingPath(dshHome) {
	return resolve(join(resolveDshHome(dshHome), STORE_FILENAME));
}
var DshAuthEveryingStore = class {
	filename;
	constructor(filename = authEveryingPath()) {
...
//#endregion
//#region src/auth.ts
async function loginOfficial(id, interaction, store = new DshAuthEveryingStore()) {
...
function fromCcSwitchConfig(id, appType, name, configText) {
	...
	const toml = isRecord$1(config) && typeof config["config"] === "string" ? parseSimpleToml(config["config"]) : {};
	...
	const provider = providerName === void 0 || !isRecord$1(toml["model_providers"]) ? {} : isRecord$1(toml["model_providers"][providerName]) ? toml["model_providers"][providerName] : {};
	...
	const baseURL = nonEmptyString(env["ANTHROPIC_BASE_URL"]) ?? nonEmptyString(env["OPENAI_BASE_URL"]) ?? nonEmptyString(env["GEMINI_BASE_URL"]) ?? nonEmptyString(provider["base_url"]) ?? nonEmptyString(toml["base_url"]);
	return source({
		...
		models,
		model: models[0],
		...modelReasoningEffort === void 0 ? {} : { modelReasoningEffort },
		api: wireApi === "responses" ? "openai-responses" : anthropicKey !== void 0 || baseURL !== void 0 && platform === "claude" ? "anthropic-messages" : official?.id === "codex" && credential?.type === "oauth" ? "openai-codex-responses" : guessApi(platform, baseURL),
...
//#endregion
//#region src/reasoning-adapter.ts
...
var CodexReasoningAdapter = class extends LlmAdapter {
	standard;
	ultra;
	ultraModels;
	constructor(standard, ultra, ultraModels) {
		super();
		this.standard = standard;
		this.ultra = ultra;
		this.ultraModels = ultraModels;
	}
	...
	async resolveModel(provider, model, signal) {
		...
		if (!this.ultraModels.get(provider)?.has(model) || reasoning === void 0) return resolved;
	...
	stream(options) {
		...
		if (!this.ultraModels.get(options.provider)?.has(options.model)) throw new LlmError(`provider "${options.provider}" model "${options.model}" does not support reasoning effort "ultra"`, "UNSUPPORTED_REASONING_EFFORT");
		return this.ultra.stream({
			...options,
			reasoningEffort: PI_XHIGH
		});
	}
};
//#endregion
//#region src/session.ts
var DshAuthEveryingSession = class {
	store;
	models;
	onChange;
	constructor(store = new DshAuthEveryingStore(), onChange) {
		this.store = store;
	...
	async persist(item) {
		...
		if (useOfficial && official.liveModelsUrl !== void 0) {
			...
			if (token !== void 0) try {
				const { fetchLiveModelIds } = await import("./live-models-l8liIaGP.js");
				available = [.../* @__PURE__ */ new Set([...available, ...await fetchLiveModelIds(official.liveModelsUrl, token)])];
			} catch {}
		}
		let modelReasoningEfforts = { ...existing?.modelReasoningEfforts ?? {} };
		if (item.origin === "CC Switch" && item.platform === "codex" && item.baseURL !== void 0) {
			const token = item.credential.type === "oauth" ? item.credential.access : item.credential.key;
			if (token !== void 0) try {
				const { fetchLiveModels, modelListingUrls } = await import("./live-models-l8liIaGP.js");
				for (const url of modelListingUrls(item.baseURL)) try {
					const live = await fetchLiveModels(url, token);
					available = [.../* @__PURE__ */ new Set([...available, ...live.map((model) => model.id)])];
					for (const model of live) if (model.reasoningEfforts !== void 0) modelReasoningEfforts[model.id] = model.reasoningEfforts;
					break;
				} catch {}
			} catch {}
		}
		...
		const route = {
			...
			models: available,
			enabled,
			...item.model === void 0 ? {} : { configuredModel: item.model },
			...item.modelReasoningEffort === void 0 ? {} : { modelReasoningEffort: item.modelReasoningEffort },
	...
	*/
	async refreshStaleCcSwitchRoutes() {
		const document = await this.store.snapshot();
		...
		for (const route of Object.values(document.routes)) {
			...
			if (item === void 0 || item.baseURL === void 0 || item.platform !== "codex") continue;
			const missingModels = route.models.length === 0;
			const staleReasoning = item.modelReasoningEffort !== void 0 && (route.configuredModel !== item.model || route.modelReasoningEffort !== item.modelReasoningEffort);
	...
	async resolveAccess(route) {
		...
		throw new LlmError(`${official?.displayName ?? route} is not imported. Open Settings → dsh-auth-everying and import a local login.`, "MISSING_CREDENTIAL");
	}
	async profiles(codexReasoningWire = "standard") {
		const document = await this.store.snapshot();
		const profiles = /* @__PURE__ */ new Map();
		const retryPolicy = resolveRetryPolicy(void 0, "dsh-auth-everying retryPolicy");
		const add = (route, displayName, provider, reasoning) => {
			profiles.set(route, {
				provider: route,
				displayName,
		...
		for (const route of Object.values(document.routes)) {
			...
			const defaultReasoning = route.sourceId.startsWith("ccswitch:codex:") ? "medium" : void 0;
...
function createDshAuthEveryingAdapterSync(session, resolveAttachments, cache) {
	const common = {
		resolveApiKey: async (provider) => session.resolveAccess(provider),
		resolveAttachments
	};
	return new CodexReasoningAdapter(new PiAiAdapter({
		profiles: () => cache.current,
		...common
	}), new PiAiAdapter({
		profiles: () => cache.ultra,
		...common
	}), cache.ultraModels);
}
//#endregion
//#region src/index.ts
const name = "llm-dsh-auth-everying";
const inject = ["llm"];
const Config = z.object({});
function apply(ctx, _config) {
	const cache = {
		current: /* @__PURE__ */ new Map(),
		ultra: /* @__PURE__ */ new Map(),
		ultraModels: /* @__PURE__ */ new Map()
	};
	let session;
	let adapter;
	let handle;
