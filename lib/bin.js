#!/usr/bin/env node
import { a as DEFAULT_CONTEXT_WINDOW, c as OFFICIAL_PLATFORMS, d as officialById, f as officialByPi, i as parseJsonDocument, l as STORE_FILENAME, m as slugify, n as isRecord$1, o as DEFAULT_MAX_TOKENS, p as officialByRoute, r as nonEmptyString, s as EXTRA_CATALOG_MODELS, t as bestSecret, u as STREAM_IDLE_TIMEOUT_MS } from "./parse-oauth-B7UckaTZ.js";
...
var DshAuthEveryingSession = class {
	...
	async persist(item) {
		...
		if (useOfficial && official.liveModelsUrl !== void 0) {
			...
			if (token !== void 0) try {
				const { fetchLiveModelIds } = await import("./live-models-D7FiyFvN.js");
		...
		if (item.origin === "CC Switch" && item.platform === "codex" && item.baseURL !== void 0) {
			const token = item.credential.type === "oauth" ? item.credential.access : item.credential.key;
			if (token !== void 0) try {
				const { fetchLiveModels, modelListingUrls } = await import("./live-models-D7FiyFvN.js");
	...
	try {
		switch (action) {
			case "status": {
				...
				for (const platform of status.platforms) process.stdout.write(`${platform.signedIn ? "in " : "out"}  ${platform.displayName}  ${platform.route}\n`);
				process.stdout.write("discovered:\n");
				for (const item of status.discovered) process.stdout.write(`  ${item.importable ? "yes" : "no "}  ${item.id}  ${item.displayName}  ${item.origin}${item.baseHost === void 0 ? "" : `  ${item.baseHost}`}\n`);
				return 0;
			}
			case "import": {
			...
			case "login": {
				...
				if (platform === void 0 || !platform.canLogin) {
				...
				try {
					await loginSession(session, platform.id, {
						prompt: async (prompt) => {
							if (prompt.type === "select") return prompt.options.find((option) => option.id.includes("oauth"))?.id ?? prompt.options[0]?.id ?? "oauth";
